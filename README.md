...
6. `Home.md`, `Search Articles.md`, and `About.md` at the vault root are special-cased to overwrite Grav's own homepage, search page, and about page directly, rather than becoming ordinary numbered pages. `Home.md`'s page is forced to a `blog` template, a categorized card grid driven entirely by each article's `taxonomy.category`, no hardcoded category list anywhere. It also carries a `content:` collection header (every top-level page, newest first) and `feed: skip: true`, the two things the community [Feed plugin](https://github.com/getgrav/grav-plugin-feed) (`bin/gpm install feed`, not part of this repo) needs to serve a real RSS/Atom feed of every article at `/.rss` / `/.atom` - `Search Articles.md` and `About.md`'s own pages get `feed: skip: true` too, so a listing/bio page never shows up as a fake "article" in the feed itself.
...
- **Date and summary**: two more optional frontmatter fields. `date:` overrides the page's displayed date (useful for content migrated from elsewhere, where the real original publish date and this pipeline's own first-commit date are different things). `summary:` is a one-line hand-written excerpt, written straight through into the generated page's own frontmatter rather than relying on Grav's own auto-computed summary, which reads a page's *rendered* content by default and can end up including a `[TOC]` widget's own link text instead of real prose. Neither has a fallback that requires the other, an article can set one, both, or neither.
- **Redirects**: an optional `redirects:` frontmatter field (a list, same two shapes as `tags:`) of old URLs that should land on this page instead of 404ing, most useful for a pre-migration permalink whose slug convention doesn't match this pipeline's own `slugify()`. Becomes Grav's own `routes: aliases:` frontmatter, a built-in feature (no plugin needed) that registers each one as a real extra route rendering this same page, without ever touching the page's own live slug.
- **Comments**: every real article (a standalone page, or one of a series' own parts) gets `comments: true` written into its frontmatter automatically, the one frontmatter switch the community [Comments Pro plugin](https://github.com/trilbymedia/grav-plugin-comments-pro) (not part of this repo) needs before its own `comments_section()` Twig call will render anything on that page. Left off a series' own overview page (it has no content of its own to comment on) and the home/search/about special pages.
- **Images**: both Obsidian's embed syntax (`![[image.png]]`, with an optional `|width` suffix which is dropped) and plain Markdown image syntax (`![alt](path/image.png)`, including Obsidian's own equivalent `|width` suffix appended to the *alt text* whenever the embed is resized in its live preview, also stripped) are rewritten to a normal `![alt](image.png)` reference, resolved by filename regardless of the original path. A URL-encoded path (`%20` for spaces, etc., common when an editor writes standard Markdown syntax for a pasted screenshot) is decoded before that lookup, so it still resolves correctly. The image itself is copied alongside the generated page, and given a lightbox link plus a size class chosen from the image's own pixel height (a short, wide screenshot doesn't end up as a tiny sliver at a fixed width).
- **SVG images** get the same lightbox and size class as a raster screenshot, but built by hand as a raw HTML anchor rather than through Grav's own image querystring action: Grav can't rasterize a real thumbnail from a vector image, so its own lightbox action would show a generic vector-file icon in the popup instead of the actual picture. The hand-built anchor's `href`/`src` is the page's own absolute route (computed the same way Grav's routing does, stripping each folder's leading `NN.` order prefix), not a bare filename - a bare filename resolves through the *browser's* own relative-URL rules instead of Grav's page-media-aware ones, landing one directory too shallow on a nested (series part) route. An SVG exported at a percentage width with no height (common from diagramming tools meant for their own inline, responsive use) also has its root `width`/`height` rewritten to explicit pixel values taken from its own `viewBox`, so it displays at a sensible size instead of the browser's generic fallback, both inline and inside the lightbox popup.
